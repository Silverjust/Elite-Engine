...
public class MapSelect {
	...
	protected void setupMapSelection() {
		...
		String[] intNames = (String[]) ContentListHandler.getModeMaps().keys().toArray(new String[size]);
		this.intNames = intNames;
		String[] names = new String[size];
		for (int j = 0; j < names.length; j++) {
			try {
				JSONObject mapData = ref.app
						.loadJSONObject("data/" + ContentListHandler.getModeMaps().getString(intNames[j]) + ".json");
		...
		if (names.length == 0) {
			...
			display.preGame.map = ContentListHandler.getModeMaps().getString(intNames[previousMap]);
		}
	...
	public void setMap(String string) {
		...
		@SuppressWarnings("unchecked")
		String[] mapArray = (String[]) ContentListHandler.getModeMaps().keys().toArray(new String[size]);
		int index = -1;
			...
			try {
				if (mapArray[i].equals(string)) {
					...
					JSONObject mapData = ref.app.loadJSONObject(
							"data/" + ContentListHandler.getModeMaps().getString(mapArray[i]) + ".json");
					index = i;
				}
			} catch (Exception e) {
				e.printStackTrace();
		...
		display.preGame.map = ContentListHandler.getModeMaps().getString(mapArray[index]);
