...
public class ActivesGrid {
	...
	public void setupSandbox() {
		...
		addBuildActive(2, 1, SandboxBuilding.class, ref.player.getNation().getNationInfo().getMainBuilding(), true);
		...
		addActive(3, 3, BuildWallActive.class, SandboxBuilding.class, Rock.class, true);
	...
	public void addActive(int x, int y, Class<? extends Active> a, boolean isUnitActive) {
		x--;
		y--;
		try {
	...
	public void addActive(int x, int y, Class<? extends Active> a, Class<?> builder, Class<? extends Entity> building,
		...
		try {
			...
			if (isUnitActive) {
				...
				buildingGrid[x][y] = (Active) ctor.newInstance(//
						new Object[] { x, y, SettingHandler.setting.buildingsShortcuts[y][x], b, builder });
			}
	...
	public void addTrainActive(int x, int y, Class<? extends Entity> trainer, Class<? extends Unit> toTrain,
		...
		try {
			...
			if (isUnitActive) {
				unitGrid[x][y] = new TrainActive(x, y, SettingHandler.setting.unitsShortcuts[y][x], u, trainer);
			} else {
				buildingGrid[x][y] = new TrainActive(x, y, SettingHandler.setting.buildingsShortcuts[y][x], u, trainer);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void addBuildActive(int x, int y, Class<?> builder, Class<? extends Building> building,
		...
		try {
			...
			if (isUnitActive) {
				unitGrid[x][y] = new BuildActive(x, y, SettingHandler.setting.unitsShortcuts[y][x], b, builder);
			} else {
				buildingGrid[x][y] = new BuildActive(x, y, SettingHandler.setting.buildingsShortcuts[y][x], b, builder);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	void addUpgradeActive(int x, int y, Class<? extends Entity> builder, Class<? extends Building> newBuilding,
		...
		try {
			...
			if (isUnitActive) {
				unitGrid[x][y] = new UpgradeActive(x, y, SettingHandler.setting.unitsShortcuts[y][x], b, oldBuilding,
						builder);
			} else {
				buildingGrid[x][y] = new UpgradeActive(x, y, SettingHandler.setting.buildingsShortcuts[y][x], b,
						oldBuilding, builder);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void selectionChange(boolean containsUnits) {
		...
		if (showUnitActives) {
			for (int x = 0; x < gridWidth; x++) {
				for (int y = 0; y < gridHeight; y++) {
					if (unitGrid[x][y] != null)
						if (Helper.listContainsInstanceOf(unitGrid[x][y].clazz, ref.updater.selected)) {
			...
			for (int x = 0; x < gridWidth; x++) {
				for (int y = 0; y < gridHeight; y++) {
					if (unitGrid[x][y] != null)
						unitGrid[x][y].setVisible(false);
					if (buildingGrid[x][y] != null)
						if (Helper.listContainsInstanceOf(buildingGrid[x][y].clazz, ref.updater.selected)) {
