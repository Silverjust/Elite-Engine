...
public class Client implements Runnable {
	...
	public Client(PApplet parent, String host, int port) {
		...
		try {
			...
			try {
				clientEventMethod = parent.getClass().getMethod("clientEvent",
			...
			try {
				disconnectEventMethod = parent.getClass().getMethod(
						"disconnectEvent", new Class[] { Client.class });
			} catch (Exception e) {
				// no such method, or an error.. which is fine, just ignore
			}
			...
			try {
				while (input != null) {
					...
					try {
						...
						System.err.println("Client SocketException: "
								+ e.getMessage());
						// the socket had a problem reading so don't try to read
						// from it again.
						stop();
						return;
					}

					// read returns -1 if end-of-stream occurs (for example if
					// the host disappears)
					if (value == -1) {
					...
					synchronized (buffer) {
						// todo: at some point buffer should stop increasing in
						// size,
					...
					if (clientEventMethod != null) {
						try {
							clientEventMethod.invoke(parent,
									new Object[] { this });
						} catch (Exception e) {
							System.err
									.println("error, disabling clientEvent() for "
											+ host);
							e.printStackTrace();
	...
	/**
	 * Return true if this client is still active and hasn't run into any
	 * trouble.
	 */
	 ...
	 * 
	 * ( end auto-generated ) <h3>Advanced</h3> Return a byte array of anything
	 * that's in the serial buffer. Not particularly memory/speed efficient,
	 * because it creates a byte array on each read, but it's easier to use than
	...
	/**
	 * <h3>Advanced</h3> Reads from the serial port into a buffer of bytes until
	 * a particular character. If the character isn't in the serial buffer, then
	 * 'null' is returned.
	 *
	 ...
	 */
	/*
	 * protected void disconnect(Exception e) { dispose(); if (e != null) {
	 * e.printStackTrace(); } }
	 */
